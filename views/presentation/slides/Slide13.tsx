...
export const Slide13: React.FC<{ isActive: boolean }> = ({ isActive }) => (
  <div className="flex flex-col items-center text-center px-4 md:px-6 pt-2 md:pt-3 pb-3 md:pb-4 select-none">
    ...
    <p className="text-sm md:text-base text-slate-500 mb-4">Respons 28 siswa terhadap platform Logi</p>

    <div className="flex flex-col lg:flex-row items-start gap-4 w-full max-w-6xl">
      <div className="bg-macaw/5 border-2 border-macaw/10 rounded-2xl p-5 md:p-6 text-center w-full lg:w-56 shrink-0 flex flex-col items-center">
        ...
        <p className="text-5xl md:text-6xl font-black text-macaw-dark font-display">
          {isActive ? '97,7' : '0'}<span className="text-3xl md:text-4xl">%</span>
        </p>
        ...
        <div className="w-full h-2 bg-white rounded-full overflow-hidden mt-3">
          <div className="h-full bg-macaw rounded-full transition-all duration-1000 ease-out" style={{ width: isActive ? '98%' : '0%' }} />
        </div>
          ...
          <div key={i} className="bg-white border border-slate-200 rounded-xl p-3 text-left">
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full bg-macaw rounded-full transition-all duration-1000" style={{ width: isActive ? `${percentages[i]}%` : '0%' }} />
              </div>
              <span className="text-[10px] font-bold text-macaw-dark tabular-nums shrink-0">{percentages[i]}%</span>
            </div>
          </div>
        ))}
      </div>
